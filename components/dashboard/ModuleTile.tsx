...
export function ModuleTile({ title, description, href, icon, onRemove }: ModuleTileProps) {
  ...
  const tileClass =
    "block rounded-2xl border border-subtle bg-theme-panel p-5 shadow-sm transition hover:-translate-y-0.5 hover:bg-theme-hover hover:shadow-md";
    ...
    <div className="relative">
      ...
      {onRemove ? (
        <button
          ...
          className="bg-elevated border-subtle absolute top-3 right-3 z-10 rounded-lg border p-1.5 text-zinc-500 shadow-sm transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/40 dark:hover:text-red-400"
